...
-(void)sendRequest
{
    [ZEUserServer  getUserInformationMessageSuccess:^(id data) {
        if ([ZEUtil isNotNull:data]) {
            [usView reloadView:data];
        }
...
-(void)initView
{
    usView = [[ZEUserCenterView alloc]initWithFrame:CGRectMake(0, NAV_HEIGHT, SCREEN_WIDTH, SCREEN_HEIGHT - NAV_HEIGHT)];
